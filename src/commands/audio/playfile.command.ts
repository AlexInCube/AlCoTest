    ...
    return {
        text_data: {
            ...
            execute: async (message: Message) => {
                ...
                if (!musicFile){
                    await message.reply({ embeds: [generateErrorEmbed(i18next.t("commands:play_file_missing_attachment", audioFormats))] })
                    return
                }

                if (!isAudioFile(musicFile.name)){
                    await message.reply({ embeds: [generateErrorEmbed(i18next.t("commands:play_file_wrong_format", audioFormats))] })
                    return
                }
        ...
        slash_data: {
            ...
            execute: async (interaction, ) => {
                ...
                if (!isAudioFile(musicFile.name)){
                    await interaction.reply({ embeds: [generateErrorEmbed(i18next.t("commands:play_file_wrong_format", audioFormats))], ephemeral: true })
                    return
                }
